...
class MyDataset_train(Dataset):
	...
	f=json.load(f)
	for line in f:
		imgs.append((os.path.join('/home/gpu2/wx/competition/data/train',line['image_id']),int(line['label_id'])))
        self.imgs = imgs
...
class MyDataset_val(Dataset):
    def __init__(self,transform=None, target_transform=None, loader=default_loader):
        f=open('scene_validation_annotations_20170908.json','r')
        imgs=[]
        f=json.load(f)
        for line in f:
                imgs.append((os.path.join('/home/gpu2/wx/competition/data/val',line['image_id']),int(line['label_id'])))
        self.imgs = imgs
        self.transform = transform
        self.target_transform = target_transform
        self.loader = loader
...
#train_loader=MyDataset(transforms.Compose([transforms.RandomSizedCrop(224),transforms.RandomHorizontalFlip(),transforms.ToTensor(),normalize,]),json='scene_train_annotations_20170904.json')
...
#    else:
    ...
    train_loader=MyDataset_train(transforms.Compose([transforms.RandomSizedCrop(224),transforms.RandomHorizontalFlip(),transforms.ToTensor(),normalize,]))
    train_loader = torch.utils.data.DataLoader(
        train_loader,
        batch_size=args.batch_size, shuffle=True,
        num_workers=args.workers, pin_memory=True)

    val_loader=MyDataset_val(transforms.Compose([transforms.RandomSizedCrop(224),transforms.RandomHorizontalFlip(),transforms.ToTensor(),normalize,]))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        save_checkpoint(model, is_best, args.arch.lower())
...
def save_checkpoint(state, is_best, filename):
    torch.save(state, filename + '_latest2.pth.tar')
    if is_best:
        shutil.copyfile(filename + '_latest2.pth.tar', filename + '_best2.pth.tar')
...
def accuracy(output, target, topk=(1,)):
    ...
    for k in topk:
        ...
        res.append(correct_k.mul_(60.0 / batch_size))
    return res
