...
model=models.__dict__['resnet50'](num_classes=int(80))
...
for line in f:
	num=num+1
	print num
	img_name=os.path.join('/home/zs/code/ai/ai_challenger_scene_test_b_20170922/scene_test_b_images_20170922',line['image_id'])
	img = Image.open(img_name)
	input_img = V(centre_crop(img).unsqueeze(0), volatile=True)
	logit = model.forward(input_img)
	h_x = F.softmax(logit).data.squeeze()
	probs, idx = h_x.sort(0, True)
        sublist.append({"image_id":line['image_id'],"label_id":list(idx[0:3])})
#img_url = 'http://places.csail.mit.edu/demo/' + img_name
#os.system('wget ' + img_url)

fp=open('submit_test_a.json','w')
json.dump(sublist,fp)
# forward pass
# logit = model.forward(input_img)
# h_x = F.softmax(logit).data.squeeze()
# probs, idx = h_x.sort(0, True)
